  ...
  if (cross_with == 0) return "Cross alone";
    ...
    for(int i = 0; i < 16; i++){
        ...
        for(int j = 0; j < 16; j++){
            ...
            if(((jbits[wolf] == jbits[goat] && jbits[wolf] != jbits[me]) || 
            (jbits[goat] == jbits[cabbage] && jbits[goat] != jbits[me])) || neighbor_label(i, j) == ""){
                ...
                nbrs[i].push_back(j);
                com = neighbor_label(i, j);
                edge_label[make_pair(i, j)] = com;
